# -*- encoding: utf-8 -*-
"""
hio.core.http.httping module
http async io (nonblocking) support
"""
...
from multidict import MultiDict as mudict, CIMultiDict as  cimdict
...
def updateQargsQuery(qargs=None, query=u'',):
    ...
    Where qargs parameter is dict of query arguments and query parameter is query string
...
def parseLeader(raw, eols=(CRLF, LF), kind="leader header line", headers=None):
    ...
    Yields cimdict of headers Otherwise as indicated by empty headers

    Raise error if eol not found before  MAX_LINE_SIZE
    """
    headers = headers if headers is not None else cimdict()
...
def parseStatusLine(line):
    ...
    version, status, reason = helping.repack(3, line.split(), default = u'')
...
def parseRequestLine(line):
    ...
    method, path, version, extra = helping.repack(4, line.split(), default = u'')
        ...
        while True:
            ...
            if not line or self.closed:  # empty line or closed so attempt dispatch
                ...
                if edata:  # data so dispatch event by appending to .events
                    ...
                    self.events.append(dict([('id', eid),
                                              ('name', ename),
                                              ('data', edata),
                                             ]))
